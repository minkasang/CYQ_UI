/* ═══════════════════════════════════════
   Liquid Glass — adapted from liquidglass-main
   Core shaders + GlassRenderer copied verbatim.
   ═══════════════════════════════════════ */
...
/* ── GlassRenderer (copied & converted to JS) ── */
class GlassRenderer {
	constructor() {
	...
	renderGlassPanel(config, elW, elH, dpr) {
		if (this.contextLost) return;
		const gl = this.gl;
		const W = this.width;
		const H = this.height;
...
class LiquidGlass {
	...
	render() {
		...
		for (const panel of this.panels) {
			...
			// 更新 canvas CSS 尺寸以包含 shadow pad，避免被父元素压缩
	...
	async changeBg(url) {
		...
		this._resizeBg();
		for (const panel of this.panels) panel._lastRender = null;
	}
...
window.LiquidGlass.DEFAULTS = DEFAULTS;
